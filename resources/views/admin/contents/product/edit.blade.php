...
@section('content')
    <div class="container-fluid py-4">
        <div class="row">
            <div class="col-12">
                <div class="card my-4">
                    <form class="card-body px-5 pb-2 row gap-3" action="{{ route('product.update', $product->id) }}" method="POST"
                        ...
                        <div class="input-group input-group-static mb-4">
                            ...
                            <select class="form-control" id="category" name="category">
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('category') == $category->id ? 'selected' : ($product->category_id == $category->id ? 'selected' : '') }}>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        @error('category')
                        ...
                        <div
                            ...
                            <textarea name="description" class="form-control" rows="5" placeholder="@lang('product.table.description')" spellcheck="false">{{ old('description') ? old('description') : $product->description }}</textarea>
                        </div>
                        @error('description')
                            <div class="invalid-feedback text-xs mb-2">
                                {{ $message }}
                        ...
                        <button type="submit" class="btn bg-gradient-success">@lang('product.button.edit')</button>
                    </form>
                </div>
            </div>
        </div>
